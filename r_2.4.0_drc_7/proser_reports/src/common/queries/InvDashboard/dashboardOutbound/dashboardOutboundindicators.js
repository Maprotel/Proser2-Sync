...
/******************************************************************** */
// MAIN FUNCTION
export async function dashboardOutboundIndicators(userSelection) {
  let result = {};
  let resume_error = false;

  let dashboardOutboundCallsIndicators = await dashboardOutboundCallsIndicatorsFunction(
    userSelection
  );
  let dashboardOutboundCurrentCallsIndicators = await dashboardOutboundCurrentCallsIndicatorsFunction(
    userSelection
  );
  ...
  let colors = [
    {
      outboundContactLevel: onColorForPercentage(
        dashboardOutboundCallsIndicators[0].outboundContactLevel,
        scale[0]
      ),
      outboundEffectiveLevel: onColorForPercentage(
        dashboardOutboundCallsIndicators[0].outboundEffectiveLevel,
        scale[0]
      ),
      callsOnQueue: onColorForCallsOnQueue(
        dashboardOutboundCurrentCallsIndicators[0].maxWaitTimeOnQue,
  ...
  result = {
    ...
    dashboardOutboundCurrentCallsIndicators,
    agentsPlannedTotal,
...
/**************************************** */
// indicators
async function dashboardOutboundCallsIndicatorsFunction(userSelection) {
  let result = null;
  let resume_error = false;
...
/**************************************** */
// current calls
async function dashboardOutboundCurrentCallsIndicatorsFunction(userSelection) {
  let result = null;
  let resume_error = false;
  let query = `
...
async function agentsConnectedTotalFunction(userSelection) {
  ...
  -- SUPERVISOR
  ${objectToJsonSqlQuery(userSelection.supervisor, "rca_people_json", "supervisor")}
...
async function agentsAuxiliarResumeFunction(userSelection) {
  ...
  -- SUPERVISOR
  ${objectToJsonSqlQuery(userSelection.supervisor, "rcb_people_json", "supervisor")}
