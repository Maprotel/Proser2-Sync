...
export async function dashboardAgentsIndicators(userSelection) {
  ...
  let dashboardAgentsCurrentCallsIndicators = await dashboardAgentsCurrentCallsIndicatorsFunction(
    userSelection
  );
  ...
  let colors = [
    {
      ...
      AgentsAttentionLevel: onColorForPercentage(
        dashboardAgentsCallsIndicators[0].AgentsAttentionLevel,
        scale[0]
      ),
      ...
      callsOnQueue: onColorForCallsOnQueue(
        dashboardAgentsCurrentCallsIndicators[0].maxWaitTimeOnQue,
  ...
  result = {
    ...
    dashboardAgentsCurrentCallsIndicators,
    agentsPlannedTotal,
...
/**************************************** */
// current calls
async function dashboardAgentsCurrentCallsIndicatorsFunction(userSelection) {
  let result = null;
  let resume_error = false;
  let query = `
-- dashboardAgentsCurrentCallsIndicatorsFunction --------------
-- FIELDS
SELECT
