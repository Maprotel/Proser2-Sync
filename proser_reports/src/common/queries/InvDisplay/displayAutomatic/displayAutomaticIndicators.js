...
export async function displayAutomaticIndicators(userSelection) {
  ...
  let displayAutomaticCurrentCallsIndicators = await displayAutomaticCurrentCallsIndicatorsFunction(
    userSelection
  );
  ...
  let colors = [
    {
      ...
      callsOnQueue: onColorForCallsOnQueue(
        displayAutomaticCurrentCallsIndicators[0].maxWaitTimeOnQue,
...
/**************************************** */
// current calls
async function displayAutomaticCurrentCallsIndicatorsFunction(userSelection) {
  let result = null;
  let resume_error = false;
  let query = `
